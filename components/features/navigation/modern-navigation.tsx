  ...
  return (
    <nav className="relative z-50 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="lg:hidden mt-4 card-glow rounded-3xl p-6 animate-fade-in border border-border/20">
              ...
              {user ? (
                <div className="pt-4 border-t border-border/30 space-y-2">
                  <div className="flex items-center space-x-3 p-3">
                    <div className="w-10 h-10 ai-gradient rounded-full flex items-center justify-center shadow-lg">
                  ...
                  {userMenuItems.map((item) => (
                    <Link
                      key={item.name}
                      href={item.href}
                      className="flex items-center space-x-3 p-3 text-foreground/80 hover:text-foreground hover:bg-background/50 rounded-xl transition-colors"
                      onClick={() => setIsMobileMenuOpen(false)}
                    >
                      <item.icon className="w-4 h-4" />
                      <span>{item.name}</span>
                  ...
                  <button className="flex items-center space-x-3 p-3 text-foreground/80 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-colors w-full">
                    <LogOut className="w-4 h-4" />
                    <span>Sign Out</span>
                  </button>
                </div>
                ...
                <div className="pt-4 border-t border-border/30 space-y-3">
                  <Link href="/auth/login" className="block w-full text-center px-6 py-3 rounded-2xl font-medium text-foreground/80 hover:text-foreground hover:bg-background/50 border border-border/30 transition-all duration-300">
                    Sign In
                  </Link>
                  <Link href="/auth/signup" className="block w-full btn-glow text-white px-6 py-3 rounded-2xl font-medium transition-all duration-300 text-center">
                    Get Started
                  </Link>
                </div>
              )}
