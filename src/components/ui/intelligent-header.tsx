...
export function IntelligentHeader({
  ...
  const { userRole, isCustomer, isProvider, isAdmin, isSuperAdmin } =
    useUserRole()
      ...
      <header className="fixed top-0 left-0 right-0 z-50 py-6 px-4 sm:px-6 lg:px-8 bg-transparent border-b border-transparent">
        <div className="responsive-container flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-xl md:text-3xl font-display text-gray-900 dark:text-white tracking-tight">
              Loconomy
            </span>
          </div>
                    ...
                    <div className="flex flex-col space-y-2">
                      <div className="flex items-center gap-3">
                        <Avatar className="h-10 w-10 border-2 border-purple-500/50">
                          ...
                          />
                          <AvatarFallback
                            className={`${getRoleBadgeColor()} text-white font-bold`}
                          >
                            {(
                              userProfile?.name ||
                              user?.email ||
                        ...
                        <div>
                          ...
                          <Badge
                            className={`${getRoleBadgeColor()} text-white text-xs flex items-center gap-1`}
                          >
                ...
                {user && (
                  <div className="flex items-center space-x-3 pb-6 border-b border-white/10">
                    ...
                    <div>
                      ...
                      <Badge
                        className={`${getRoleBadgeColor()} text-white text-xs flex items-center gap-1 w-fit`}
                      >
                        <RoleIcon className="w-3 h-3" />
                        {userRole?.charAt(0).toUpperCase() +
                          userRole?.slice(1) || 'User'}
